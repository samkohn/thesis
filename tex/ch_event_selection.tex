...
This process is known as production, and the resulting data sets
...
The production dataset used in this analysis is known as P17B,
...
\Cref{sec:DT_cut} describes the signal enhancement from
requiring a small displacement in distance and time between
the prompt and delayed sub-events that form each coincidence.
Lastly, \cref{sec:energy_cuts} details the energy cuts
which remove background events
and nGd-IBD events, where the neutron captured on Gd in the GdLS volume.
...
analysis (\cref{sec:acc}), overlapping windows are combined
...
\missingfigure{Accidentals sample plots for each AD}
...
Referencing the first line\todo[inline]{Errors on $R_s$ and $R_{\text{\fold{1}}}$
    are correlated
} of \cref{eq:racc},
...
The uncertainty on $\varepsilon_{\text{DT,\,acc}}$ \todo{$\varepsilon_{\text{DT,\,acc}}$}
...
\todo{Table of total $N_{\text{corr}}$ for each AD}
