...
and a significantly relaxed DT cut of \SI{10}{\meter},
encompasing \SI{>99.99}{\percent} of events:
\begin{equation}
    \varepsilon_{\text{DT}} = \frac{N(\text{DT} < \SI{800}{\milli\meter})}{
    N(\text{DT} < \SI{10000}{\milli\meter})}.
\end{equation}
...
The DT cut absolute efficiency for each AD is shown in \cref{fig:DT_eff},
although it is not used as an input to the \thetaot{} analysis.
...
was determined from the data by examining the variation
in measured absolute efficiency.
...
The sensitivity arose when computing $N(\text{DT} < \SI{10000}{\mm}$
using a histogram representing the accidentals-subtracted
distribution of DT values.
...
of the background to subtract at a far site AD
yielded an impact of \SI{2.1}{\percent} in the extracted DT cut efficiency,
amplifying the error by a factor of over 200.
(The near site ADs were sensitive by a factor of $\sim30$.)

Two special selections were used to reduce the impact of the
...
The nGd subset yielded a variation of \SI{0.18}{\percent}
in DT cut absolute efficiencies between ADs,
while the $E_p > \SI{3.5}{\MeV}$ yielded a variation of \SI{0.94}{\percent}.
...
of prompt and delayed positions and energies was represented,
allowing the DT efficiency variation to be accurately computed
with a minimal impact from the accidental background.
Conservatively, the half-range of the $E_p > \SI{3.5}{\MeV}$ subset,
\SI{\pm0.5}{\percent}, was
...
and have a lifetime of \SI{20.2}{\ms}.
...
could not effectively discriminate the additional events
on top of the uncorrelated events (i.e. IBDs and accidentals).
...
First, $N_\text{bkg}$ included many \li{} events which were vetoed
by the muon veto in the nominal selection.
The fraction of surviving events was computed
using the known decay lifetimes
of \li{} (\SI{257.23}{\ms}) and \he{} (\SI{171.60}{\ms})
and the length of the muon veto windows
(\SI{800}{\us} for $E_\mu < \SI{2.5}{\GeV}$
and \SI{1}{\s} for $E_\mu > \SI{2.5}{\GeV}$).
Negligibly few \li{} events (\SI{<0.4}{\percent})
were vetoed by the \SI{800}{\us} criterion,
but \SI{98}{\percent} were vetoed by the \SI{1}{\s} criterion
for high energy muons.
After this correction was applied,
the event count was further corrected
by the difference in prompt energy cut efficiencies
between the \SI{3.5}{\MeV} cut used in this study
and the \SI{1.5}{\MeV} cut used for neutrino candidates.
The measurement of the \li{} spectrum and prompt efficiency
will be discussed below.
Lastly, the number of events in a given experimental hall
was divided into counts for each AD
based on the ratio of DAQ livetimes among the ADs within that hall.
All ADs within a hall shared the same effective rate of \li{} events
(corrected for slight differences in $\varepsilon_\mu$ and $\varepsilon_m$)
and the same relative uncertainty.
...
is normalized to the fitted value for $N_\text{uncorr}$
