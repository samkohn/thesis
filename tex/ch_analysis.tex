...
can be found in Table X \todo{Pull parameters table}.
...
and the minimization procedure described in \cref{sec:fitter}
(collectively, the ``fitter'')
were validated against an existing Monte Carlo simulation
designed for the nGd analysis.
...
(\cref{subsec:validation_nominal}).
...
(\cref{subsec:validation_final}).

\subsection{No fluctuations}
\label{subsec:validation_nominal}

\subsection{Statistical fluctuations}
\label{subsec:validation_stat}

\subsection{Systematic uncertainties}
\label{subsec:validation_syst}

\subsection{Final validation}
\label{subsec:validation_final}
