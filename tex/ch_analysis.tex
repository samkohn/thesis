...
\begin{figure}
    \centering
    \includegraphics[height=0.9\textheight]{ch_analysis/near_far_cartoon}
...
The contribution of each pull parameter $\nu_i$ to the $\chi^2$ total
is defined (using the notation from \cref{eq:chisquare_generic}) as
\begin{equation}\label{eq:pull_contrib}
    \delta\chi^2 = \frac{\nu_i^2}{\tilde{\sigma}_i^2}.
\end{equation}
The contributions of the 48 systematics nuisance parameters
to the total $\chi^2$
are shown in \cref{fig:syst_pulls_plot}.
16 parameters contributed more than $10^{-6}$ to the total;
those parameters are listed with additional details
in \todo{Table of best fit pulls}.%\cref{tab:syst_best_fits}.

\begin{figure}
    \missingfigure{Best fit pull parameters}
    \caption{Best fit pull parameters}
